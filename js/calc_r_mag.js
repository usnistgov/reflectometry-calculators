...
import ModulePromise from "./refl/magrefl.js";
...
function calculate_U1_U3(H, rhoM, thetaM, Aguide) {
  ...
  var new_my =
    sld_m_z * Math.sin(radians(Aguide)) + sld_m_y * Math.cos(radians(Aguide));
  var new_mz =
    sld_m_z * Math.cos(radians(Aguide)) - sld_m_y * Math.sin(radians(Aguide));
  sld_m_y = new_my;
  sld_m_z = new_mz;
  sld_h_x = 0.0;
  sld_h_y = 0.0;
  ...
  sld_b = Math.sqrt(
    Math.pow(sld_b_x, 2) + Math.pow(sld_b_y, 2) + Math.pow(sld_b_z, 2),
  );
...
import { magnetic_wavefunction } from "./magnetic_wf_B.esm.js";
...
var calc_r_cpplib = function (sld, qmin, qmax, qstep, H, AGUIDE) {
  ...
  // run calculation with AGUIDE set to zero, as we are rotating M in u1, u3 calculation.
  var R = refl_module.magrefl_less(
    depth,
    sigma,
    rho,
    irho,
...
self.onmessage = function (event) {
  ...
  // const r = calc_r_js(sld, qmin, qmax, qstep, AGUIDE);
  self.postMessage(r);
  return;
};
