...
div_form_line = function(x0, y0, x1, y1, qx, qy) {
    //var qxl = qx[..., newaxis] 
    //var qyl = qy[..., newaxis]
    var dx = x1 - x0;
    var dy = y1 - y0;
  
    var qlensq = qx*qx + qy*qy;
    var result;
    //var result = zeros(qlensq.shape + x0.shape, dtype=complex128)
    if (qlensq == 0.0) { 
        result = x0 * dy - y0 * dx;
    }
    else {
        var prefac = 1.0/(qlensq) * (-qx*dy + qy*dx)/(qx*dx + qy*dy);
        result = Complex.multiply(prefac, Complex.subtract(Complex.exp(new Complex(0.0, (qx*x1 + qy*y1))), Complex.exp(new Complex(0.0, (qx*x0 + qy*y0)))));
    }
    return result
}
